...
export function BoxRisk({ stock }: { stock: Stock }) {
  ...
  for (const r of stock.riskFlags) counts[r.severity]++;
    ...
    <>
      ...
      <TableModal open={expanded} onOpenChange={setExpanded} title="Risk & Alert Flags" subtitle={`${stock.symbol} · ${stock.name}`}>
        ...
        <div className="mt-3 text-[9px] text-text3">
          Severity reflects current flagged conditions only. Probability/impact scoring requires the full risk-engine backend (Phase 6) — not yet wired.
        </div>
      </TableModal>
    </>
  );
